...
// Search for Visual Studio 2017-2022
	var nVersionsNew = 3;
		...
		try
		{
			...
			while(!fileLog.AtEndOfStream)
			{
				...
				if(bRet)
				{
					...
					var strDevEnvExe = FileSys.BuildPath(strLine, "Common7\\IDE\\devenv.exe");
					if(FileSys.FileExists(strDevEnvExe))
					{
						// Notify the user that we're about to run "devenv.exe /updateconfiguration"
						var strMsgWait = "Configuring " + astrVersionsNew[j] + "...\n\n" +
										"This process may take a few minutes.\n" +
										"Please wait until the configuration is complete.";
						WSShell.Popup(strMsgWait, 0, "WTL Wizard Setup", 64);
						if(bDebug)
							WScript.Echo("Running: " + strDevEnvExe + " /updateconfiguration");
						// Run (0=hidden window, true=wait)
						WSShell.Run("\"" + strDevEnvExe + "\" /updateconfiguration", 0, true);
					}
					else
					{
						if(bDebug)
							WScript.Echo("WARNING: Cannot find devenv.exe for " + astrVersionsNew[j]);
					}
				}
			}
